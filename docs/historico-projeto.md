...
> **Importante**: os arquivos `.gs` do Apps Script (Auth.gs, Router.gs, Sync.gs, ImportB3.gs, Macros.gs) **não fazem parte deste repositório git**. Eles são entregues sempre via arquivo pro Tiago colar manualmente no editor do Apps Script (Google não oferece versionamento git nativo pra Apps Script nesse fluxo). A versão mais recente de cada um foi entregue como arquivo na conversa — vale conferir com o Tiago se ele quer que a gente também guarde uma cópia deles aqui no repo (fora do fluxo de deploy) só como histórico/backup.
...
- **aux_historico-patrimonio**: aba criada pelo Tiago, guarda o patrimônio diário por ativo (Data | Ticker | Classe | Cotas | Preço | Valor | Câmbio | Valor BRL), **append-only** — uma linha já gravada nunca é recalculada.
...
- **Auxiliar_app**: célula de rascunho (`AZ1`) usada como scratch pelo mecanismo de busca do GOOGLEFINANCE — nunca deve ser usada pra outra coisa.
...
- Arquivos `.gs` do Apps Script: gerados na nuvem → entregues como arquivo na conversa → Tiago cola manualmente no editor do Apps Script (não fazem parte do git).
- Arquivos de front-end/documentação (`.html`, `.md`) que entram no repositório: gerados na nuvem → entregues como arquivo → escritos no repositório real (`/Users/tiagosilva/Documents/Desenvolvimento/investiments`) via ponte de dispositivo → `git add` + `git commit` (mensagens sem acento, heredoc) rodado no Mac do Tiago via shell remoto.
