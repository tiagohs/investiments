...
- **`gravarSnapshotResumoHoje_()`** (SnapshotResumoDiario.gs) — SEM gatilho próprio (pedido do Tiago, 18/09/2026): chamada como passo extra dentro do próprio `gatilhoDiario()` (~10h), logo depois de `atualizarHistorico`, em try/catch separado. Grava/atualiza (idempotente por data, só regrava se o valor mudou) 1 linha em Auxiliar_app!E:J com os 4 valores de patrimônio que `montarHome_()` já lê ao vivo (Total/Longo Prazo/Nacional/Renda Emergencial), datada com o dia do PRÓPRIO gatilho (não "ontem"). Caminho separado pro comparativo "ontem era" (resumo cards, tela Início) — não depende da série histórica combinada (`montarSerieHistoricoInicio_`), que tem 3 fontes com gatilhos em horários diferentes e pode ficar incompleta no dia mais recente sem erro nenhum. Ressalva consciente: às ~10h o valor "ao vivo" ainda está próximo do fechamento de ONTEM (B3 acabou de abrir) — imprecisão pequena e que não acumula dia a dia, aceita de propósito pra reaproveitar o gatilho existente em vez de criar um novo.
